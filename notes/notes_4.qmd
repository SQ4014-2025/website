...
-   In the terminal window, type `code itunes.py`. Code as follows:
    ...
    Notice how the returned value of `requests.get` will be stored in `response`. Having read the Apple documentation about this API, you'd know that what is returned is a JSON file. Running `python itunes.py weezer`, you will see the JSON file returned by Apple. However, the JSON response is converted by Python into a dictionary. Looking at the output, it can be quite dizzying!
